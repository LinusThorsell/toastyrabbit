...
pub async fn delete(State(state): State<AppState>, Json(payload): Json<structs::TableSpec>) -> () {
    let migration_string = create_migration_file::build_drop(&payload)
    ...
    let _ = File::create(format!(
        "migrations/U{}__drop_table_{}.sql",
        Utc::now().timestamp(),
        payload.table
    ))
    .await
